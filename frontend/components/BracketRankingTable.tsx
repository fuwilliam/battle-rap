...
export function BracketRankingTable({ rows }: { rows: BracketRankingRow[] }) {
  // canonical rank from the incoming (championships, final_fours, win_rate,
  // wins desc) order; stays glued to each artist even when re-sorted
  ...
  if (rows.length === 0) {
    return (
      <p className="rounded-2xl border border-white/10 px-4 py-8 text-center text-sm text-white/50">
        No bracket runs played yet.
      </p>
    );
  }
            ...
            {visible.map((r) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-right tabular-nums">
                  {r.championships > 0 ? `🏆 ${r.championships}` : 0}
                </td>
